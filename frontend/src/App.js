...
import { Search, X, Home, MapPin, Mail, Phone, Sparkles, Eye, Users, Loader2, BarChart3, Share2, MessageCircle, Send } from "lucide-react";
...
const LISTING_TYPES = {
  offering: { label: "Offering", emoji: "🏠", color: "#10B981", bg: "#D1FAE5", text: "#065F46", gradient: "linear-gradient(135deg, #10B981 0%, #059669 100%)" },
  looking: { label: "Looking", emoji: "👀", color: "#8B5CF6", bg: "#EDE9FE", text: "#5B21B6", gradient: "linear-gradient(135deg, #8B5CF6 0%, #7C3AED 100%)" },
  sharing_rent: { label: "My Rent", emoji: "📊", color: "#F97316", bg: "#FFF7ED", text: "#9A3412", gradient: "linear-gradient(135deg, #F97316 0%, #EA580C 100%)" },
};

const createIcon = (type) => {
  const config = LISTING_TYPES[type] || LISTING_TYPES.sharing_rent;
  return L.divIcon({
    className: "custom-div-icon",
    html: `<div style="width:32px;height:32px;background:${config.gradient};border-radius:50% 50% 50% 0;transform:rotate(-45deg);box-shadow:0 4px 12px ${config.color}66;border:3px solid white;display:flex;align-items:center;justify-content:center;"><div style="transform:rotate(45deg);font-size:14px;">${config.emoji}</div></div>`,
    iconSize: [32, 32],
    iconAnchor: [16, 32],
  });
};
...
const createPinIcon = () => L.divIcon({
  className: "custom-div-icon",
  html: `<div style="width:40px;height:40px;background:linear-gradient(135deg, #F97316, #EA580C);border-radius:50%;box-shadow:0 4px 20px rgba(249,115,22,0.5);border:4px solid white;display:flex;align-items:center;justify-content:center;animation:pulse 1.5s infinite;"><div style="color:white;font-size:18px;">+</div></div>`,
  iconSize: [40, 40],
  iconAnchor: [20, 20],
});
...
const DashboardModal = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      setIsLoading(true);
      axios.get(`${API}/stats/dashboard`).then(r => setStats(r.data)).catch(() => toast.error("Failed to load stats")).finally(() => setIsLoading(false));
    }
  }, [isOpen]);
    ...
    <div className="form-modal" onClick={onClose} data-testid="dashboard-modal">
      <div className="form-content animate-slide-up" style={{ maxWidth: '600px' }} onClick={e => e.stopPropagation()}>
        <div className="form-header bg-gradient-to-r from-orange-500 to-orange-600 text-white">
          <div>
            <h2 className="text-xl font-bold flex items-center gap-2"><BarChart3 size={24} /> Market Insights</h2>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 rounded-xl p-4 text-center">
                  <div className="text-2xl font-bold text-emerald-700">{stats?.lowest_rent?.rent_amount ? `€${stats.lowest_rent.rent_amount}` : '—'}</div>
                ...
                <div className="bg-gradient-to-br from-red-50 to-red-100 rounded-xl p-4 text-center">
                  <div className="text-2xl font-bold text-red-700">{stats?.highest_rent?.rent_amount ? `€${stats.highest_rent.rent_amount}` : '—'}</div>
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={stats?.avg_by_neighborhood?.slice(0, 6) || []} layout="vertical">
                      ...
                      <Bar dataKey="avg_rent" radius={[0, 4, 4, 0]}>
                        {(stats?.avg_by_neighborhood || []).slice(0, 6).map((_, i) => (
...
const ListingCard = ({ listing, onView, onShare }) => {
  const typeConfig = LISTING_TYPES[listing.listing_type] || LISTING_TYPES.sharing_rent;

  return (
    <div className={`listing-card ${listing.listing_type}`} data-testid={`listing-card-${listing.id}`}>
      <div className="flex items-start justify-between mb-3">
        <span className="badge" style={{ background: typeConfig.bg, color: typeConfig.text }}>
          {typeConfig.emoji} {typeConfig.label}
        </span>
        <div className="flex gap-1">
          {listing.apartment_type && <span className="badge badge-default">{listing.apartment_type}</span>}
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 mb-3 text-sm text-gray-500">
        <MapPin size={14} style={{ color: typeConfig.color }} />
        <span className="font-medium">{listing.neighborhood}</span>
        {listing.building_type && <span className="ml-auto badge badge-sm">{listing.building_type}</span>}
      </div>

      {(listing.description || listing.ai_description) && (
        <p className="text-sm text-gray-600 mb-3 line-clamp-2">
          {listing.description || listing.ai_description}
          {listing.ai_description && !listing.description && <span className="ml-1 text-xs text-purple-500">AI</span>}
        </p>
      )}
      ...
      <div className="flex items-center gap-2 pt-3 border-t border-gray-100">
        ...
        <div className="ml-auto flex gap-2">
          <button className="icon-btn" onClick={e => { e.stopPropagation(); onShare(listing); }} data-testid={`share-${listing.id}`} title="Share on WhatsApp">
            <Share2 size={14} />
          </button>
          {listing.comments?.length > 0 && (
            <span className="text-xs text-gray-400 flex items-center gap-1"><MessageCircle size={12} /> {listing.comments.length}</span>
          )}
          <button className="text-xs text-gray-500 hover:text-orange-500 flex items-center gap-1" onClick={() => onView(listing)} data-testid={`view-${listing.id}`}>
            <Eye size={14} /> View
          </button>
        </div>
      </div>
...
const CreateListingModal = ({ isOpen, onClose, pinLocation, neighborhoods, onSubmit }) => {
  ...
  const getAIDescription = async () => {
    ...
    } catch { toast.error("Couldn't generate description"); }
    finally { setIsGeneratingDesc(false); }
  };

  const getAIPrice = async () => {
    if (!formData.apartment_size) { toast.error("Enter apartment size first"); return; }
    ...
    } catch { toast.error("Couldn't get price suggestion"); }
    finally { setIsGettingPrice(false); }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      onClose();
      toast.success("Posted! Thanks for sharing.");
    ...
    <div className="form-modal" onClick={onClose} data-testid="create-listing-modal">
      <div className="form-content animate-slide-up" onClick={e => e.stopPropagation()}>
        <div className="form-header">
          <div>
            <h2 className="text-xl font-bold text-gray-900">{stepLabels[step]}</h2>
            <p className="text-sm text-gray-500 mt-1">
              <MapPin size={12} className="inline" /> {pinLocation?.neighborhood || 'Berlin'} &middot; Step {step}/{totalSteps}
            </p>
          </div>
          ...
          {step === 1 && (
            <div className="space-y-3">
              {Object.entries(LISTING_TYPES).map(([key, config]) => (
                <div
                  key={key}
                  className={`type-card ${formData.listing_type === key ? `active ${key}` : ''}`}
                  onClick={() => setFormData({ ...formData, listing_type: key })}
                  data-testid={`type-${key}`}
                >
                  <div className={`type-icon ${key}`}>{config.emoji}</div>
                  <div>
                    <div className="font-semibold">{
                      key === 'offering' ? "I'm Offering a Place" :
                      key === 'looking' ? "I'm Looking for a Place" :
                      "Just Sharing What I Pay"
                    }</div>
        ...
        <div className="form-footer">
          ...
          {!isLastStep ? (
            ...
            <button type="button" className="btn btn-primary ml-auto" onClick={handleSubmit} disabled={isSubmitting} data-testid="submit-listing-btn">
              {isSubmitting ? 'Posting...' : formData.listing_type === 'sharing_rent' ? 'Share My Rent' : 'Post Listing'}
            </button>
          )}
        </div>
      </div>
...
const ViewListingModal = ({ listing, isOpen, onClose, onCommentAdded }) => {
  ...
  const handleAddComment = async () => {
    ...
    setIsSubmitting(true);
    try {
      const r = await axios.post(`${API}/listings/${listing.id}/comments`, { text: newComment, author_name: authorName || 'Anonymous' });
      setComments([...comments, r.data.comment]);
      setNewComment('');
      setAuthorName('');
      toast.success("Comment added!");
      if (onCommentAdded) onCommentAdded(listing.id, r.data.comment);
  ...
  return (
    <div className="form-modal" onClick={onClose} data-testid="view-listing-modal">
      <div className="form-content animate-slide-up" onClick={e => e.stopPropagation()}>
        <div className="form-header" style={{ background: typeConfig.color, color: 'white' }}>
          <div>
            <span className="text-sm opacity-80">{typeConfig.emoji} {typeConfig.label}</span>
            <h2 className="text-xl font-bold">{listing.apartment_type} in {listing.neighborhood}</h2>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={handleShare} className="p-2 hover:bg-white/20 rounded-full" title="Share on WhatsApp"><Share2 size={20} /></button>
            <button onClick={onClose} className="p-2 hover:bg-white/20 rounded-full"><X size={20} /></button>
          </div>
        </div>
        ...
        <div className="form-body max-h-[60vh] overflow-y-auto">
          ...
          <div className="border-t border-gray-100 pt-4 mt-4">
                ...
                {comments.map((c, idx) => (
                  <div key={idx} className="bg-gray-50 rounded-lg p-3">
                    <div className="flex items-center justify-between mb-1">
                      <span className="font-medium text-sm text-gray-700">{c.author_name}</span>
            ...
            <div className="space-y-2">
              <input type="text" placeholder="Your name (optional)" value={authorName} onChange={e => setAuthorName(e.target.value)} className="w-full p-2 text-sm border border-gray-200 rounded-lg focus:border-orange-500 focus:ring-1 focus:ring-orange-500" />
              <div className="flex gap-2">
                <input type="text" placeholder="Write a comment..." value={newComment} onChange={e => setNewComment(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleAddComment()} className="flex-1 p-2 text-sm border border-gray-200 rounded-lg focus:border-orange-500 focus:ring-1 focus:ring-orange-500" data-testid="comment-input" />
                <button onClick={handleAddComment} disabled={isSubmitting || !newComment.trim()} className="btn btn-primary px-4" data-testid="submit-comment-btn">
        ...
        <div className="form-footer">
          {listing.contact_email && <a href={`mailto:${listing.contact_email}`} className="btn btn-secondary flex-1 justify-center"><Mail size={18} /> Email</a>}
          {listing.contact_phone && <a href={`tel:${listing.contact_phone}`} className="btn btn-primary flex-1 justify-center"><Phone size={18} /> Call</a>}
          {!listing.contact_email && !listing.contact_phone && (
            <div className="text-center text-gray-400 text-sm w-full">
              {listing.listing_type === 'sharing_rent' ? 'Anonymous rent data — no contact info' : 'No contact information provided'}
            </div>
          )}
        </div>
      </div>
...
const FilterBar = ({ filters, setFilters, neighborhoods }) => (
  <div className="filter-bar" data-testid="filter-bar">
    ...
    <div className="segmented-control" data-testid="filter-listing-type">
      ...
      <button className={filters.listingType === 'sharing_rent' ? 'active' : ''} onClick={() => setFilters({ ...filters, listingType: 'sharing_rent' })} data-testid="filter-sharing">📊</button>
    </div>
...
const Stats = ({ listings }) => {
  ...
  const sharing = listings.filter(l => l.listing_type === 'sharing_rent').length;

  return (
    <div className="stats-bar" data-testid="stats-summary">
      <div className="stat-item"><span className="stat-value">{listings.length}</span><span className="stat-label">Total</span></div>
      <div className="stat-item"><span className="stat-value text-emerald-600">{offerings}</span><span className="stat-label">Offering</span></div>
      <div className="stat-item"><span className="stat-value text-purple-600">{looking}</span><span className="stat-label">Looking</span></div>
      <div className="stat-item"><span className="stat-value text-orange-600">{sharing}</span><span className="stat-label">Rents</span></div>
    </div>
  );
};
...
function App() {
  ...
  const fetchListings = useCallback(async () => {
    try {
      ...
      if (filters.searchQuery) {
        ...
        data = data.filter(l => l.neighborhood.toLowerCase().includes(q) || l.apartment_type.toLowerCase().includes(q) || (l.description && l.description.toLowerCase().includes(q)));
      }
      setListings(data);
  ...
  const handleMapClick = (latlng) => {
    if (showCreateModal || selectedListing || showDashboard) return;
  ...
  const handleCommentAdded = (listingId, comment) => {
    setListings(prev => prev.map(l => l.id === listingId ? { ...l, comments: [...(l.comments || []), comment] } : l));
  };
  ...
  return (
    <div className="app-container" data-testid="app-container">
      ...
      {/* Tap-to-add hint banner */}
      <div className="tap-hint-banner" data-testid="tap-hint">
        <MapPin size={14} /> <span>Click anywhere on the map to share your rent, list a place, or post what you're looking for</span>
      </div>
      ...
      <main className="main-content">
        <div className="map-section" data-testid="map-section">
          <MapContainer center={BERLIN_CENTER} zoom={BERLIN_ZOOM} style={{ height: '100%', width: '100%', cursor: 'crosshair' }}>
            ...
            {listings.map(listing => (
              <Marker key={listing.id} position={[listing.lat, listing.lng]} icon={createIcon(listing.listing_type)} eventHandlers={{ click: () => setSelectedListing(listing) }}>
                <Popup>
          ...
          <div className="map-legend" data-testid="map-legend">
            <div className="flex items-center gap-2 mb-1"><div className="w-3 h-3 rounded-full" style={{ background: '#10B981' }}></div><span>Offering</span></div>
            <div className="flex items-center gap-2 mb-1"><div className="w-3 h-3 rounded-full" style={{ background: '#8B5CF6' }}></div><span>Looking</span></div>
            <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full" style={{ background: '#F97316' }}></div><span>Rent Data</span></div>
          </div>

          <div className="built-by" data-testid="built-by">Built by <span>Sachin</span></div>
        </div>
        ...
        <div className="listings-section" data-testid="listings-section">
          ...
          {isLoading ? (
            <div className="empty-state"><div className="spinner"></div><p className="mt-4 text-sm font-medium">Loading listings...</p></div>
          ) : listings.length === 0 ? (
            <div className="empty-state" data-testid="empty-state">
              <div className="w-16 h-16 rounded-full bg-gray-100 flex items-center justify-center mb-4"><Users size={32} className="text-gray-400" /></div>
              <p className="font-semibold text-gray-700">No listings yet</p>
              <p className="text-sm text-gray-500 mt-1">Click on the map to add the first one!</p>
            </div>
          ) : (
            <div data-testid="listings-grid" className="pb-24 lg:pb-4">
              {listings.map(listing => <ListingCard key={listing.id} listing={listing} onView={setSelectedListing} onShare={handleShare} />)}
            </div>
          )}
        </div>
      </main>
